...
z_download() {
    ...
    local xuanxuan_base_name="xxb.${version}.bundle.zbox_64.tar.gz"
    ...
    "zdoo")
        ...
        unzip -qq -d ${directory} /tmp/"${1}" \
        && rm -rf /apps/zdoo/www/data \
        && rm /tmp/"${1}"
        ;;
    "xuanxuan")
        wget --no-check-certificate --quiet --output-document=/tmp/"${1}" "${XUANXUAN_URL}/${version}/${xuanxuan_base_name}"
        tar xzf /tmp/"${1}" -C /opt/ \
        && rm /tmp/"${1}"
        ;;
    *)
